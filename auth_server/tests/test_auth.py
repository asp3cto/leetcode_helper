...
async def test_register_unique(ac: AsyncClient):
    ...
    assert response.json()["username"] == "unique_nickname" and response.json()["email"] == "test@example.com"
...
async def test_login_valid_credentials(ac: AsyncClient):
    ...
    response = await ac.post("/auth/login/", data={"username": "unique_nickname", "password": "unique_user_passwd"}, headers=headers)
    
    assert response.status_code == expected_response_status_code
    assert response.cookies.get("access_token")
    assert response.json().get("access_token") == response.cookies.get("access_token") and response.json().get("token_type") == "Bearer"
...
async def test_login_invalid_passwd(ac: AsyncClient):
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = await ac.post("/auth/login/", data={"username": "unique_nickname", "password": "unique_user_wrong_passwd"}, headers=headers)
    assert response.status_code == expected_response_status_code
...
async def test_login_invalid_login(ac: AsyncClient):
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    response = await ac.post("/auth/login/", data={"username": "unique_nickname_wrong", "password": "unique_user_passwd"}, headers=headers)
    assert response.status_code == expected_response_status_code
...
async def test_login_cookie_deleted(ac: AsyncClient):
    ...
    response = await ac.post("/auth/login/", data={"username": "unique_nickname", "password": "unique_user_passwd"}, headers=headers, cookies=cookies,)
